...
int need_to_link(main_file_name) char *main_file_name;
{
    ...
    */
    ...
    if (end < 3)
        cannot_link(main_file_name, "", "No base name: %s%s");
    end--;

    if (stat(main_file_name, &c_stat))
        cannot_link(main_file_name, strerror(errno), "Could not access %s: %s");
    strcpy(file_name, main_file_name);
    file_name[end] = 'K';
    if (stat(file_name, &k_stat))
        return clear_y(main_file_name);
    file_name[end] = '\0';
    strcat(file_name, "LINK");
    if (stat(file_name, &link_stat))
        return clear_y(main_file_name);
    ...
    for (i = 0; i < n; i++) {
        ...
        if (stat(file_name, &src_stat))
            cannot_link(file_name, strerror(errno), "Could not access %s: %s");
        file_name[end] = 'T';
        ...
        if (stat(file_name, &lif_stat))
            cannot_link(file_name, strerror(errno), "Could not access %s: %s");
        if ((h_stat.st_mtime < src_stat.st_mtime) || (t_stat.st_mtime < src_stat.st_mtime) ||
...
XtIntervalId *tt;
{
    ...
    if (n)
        XawListChange(id->list, id->progs, 0, 0, False);
    else
        exit(0);
}
...
void strech_to(frame, w, goal) Widget goal, w, frame;
{
    ...
    XtVaGetValues(frame, XtNwidth, &width, XtNdefaultDistance, &dist, NULL);
    XtVaGetValues(goal, XtNwidth, &width_goal, XtNx, &x, XtNborderWidth, &bw, NULL);
    XtVaGetValues(w, XtNwidth, &width_w, XtNborderWidth, &bw_w, NULL);

    width_w = width_w + width - (x + 2 * bw + width_goal + dist);

    XtUnmanageChild(w);
    XtVaSetValues(w, XtNwidth, (XtArgVal)width_w, NULL);
    XtManageChild(w);
}
...
XtPointer w_data;
{
    ...
    sprintf(
        buff, "%s/helpu -geom +20+20 %s %s/select.help &", HOME, "-title \"Analyzer Help\" ", HOME);
    system(buff);
}
...
void v_wrap(p, lw) Widget p, lw;
{
    ...
    XtVaGetValues(p, XtNheight, &hp, NULL);

    XtVaGetValues(lw, XtNheight, &hl, NULL);

    XtUnmanageChild(p);
    XtUnmanageChild(lw);
    XtVaSetValues(p, XtNheight, hl, NULL);
    XtManageChild(p);
    XtManageChild(lw);
}
...
XtAppContext ac;
{
    ...
    selected  = make_list_box(frame, help, status, "select", "Main Program Files", &id);
    id.status = status;
    XtAddCallback(XtNameToWidget(selected, "*selectList"), XtNcallback, call_slice, &id);
    ...
    */
    ...
    set_button_help(help, "Click to pop up a window describing selection");
