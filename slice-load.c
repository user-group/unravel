...
*/
	...
	for (i = 2; i <= offset; i++){
		...
		if ((dot - vars[id+i].name) != (at - vars[id+1].name)){
			j = i + 1;
			if (j <= offset)dot = strrchr (vars[id+j].name,'.');
			while ((j <= offset) &&
				(dot - vars[id+j].name) != (at - vars[id+1].name)){
				j++;
				if (j <= offset) dot = strrchr (vars[id+j].name,'.');
			}
...
close_gdefs()
{
	...
	for (proc = 1; proc <= n_procs; proc++){
		...
		if(!is_lib_proc(proc))while (more){
			more = 0;
			if (procs[proc].cdefs){
				chain = -1;
				while ((chain = get_next_member(
					procs[proc].cdefs,chain)) >= 0){
					chain_resolve_set (chain,proc);
					while (chain_resolve_get (&id,&pid)){
						if(v_opt)printf ("got %s\n",var_name(pid,id));fflush(stdout);
						if(c_opt)printf (
							"chain %d resolves to (%d,%d) %s %c\n",
							chain,pid,id,var_name(pid,id),
							(pid?procs[pid].locals[id].is_pointer:
							globals[id].is_pointer)?'P':'S');
						if(offset_check(pid,id,0)){
						if (pid){
							if (pid != proc)
								more += 
									add_to_id_set (
										&procs[proc].other_defs,pid,id);
						}
						else {
							if (!procs[proc].global_defs)
								procs[proc].global_defs =
									create_bit_set (n_globals+1);
							...
							if(v_opt)printf ("id range error\n");
						}
					}
				}
			}
			if (procs[proc].global_idefs){
				var = -1;
				while ((var = get_next_member(
					procs[proc].global_idefs,var)) >= 0){
					...
					while (objs){
						...
						if (addrs[aix].pid == 0){
							if (!procs[proc].global_defs)
								procs[proc].global_defs =
									create_bit_set (n_globals+1);
							if (!is_bit_on(procs[proc].global_defs,
								addrs[aix].id)){
								bit_on(procs[proc].global_idefs,
									addrs[aix].id);
								bit_on(procs[proc].global_defs,
									addrs[aix].id);
								more++;
							}
						}
							...
							old = more;
							more += 
								add_to_id_set (&procs[proc].other_defs,
								addrs[aix].pid,addrs[aix].id);
						}
						objs = objs -> next;
					}
				}
			...
			if (procs[proc].local_idefs){
				var = -1;
				while ((var = get_next_member(
					procs[proc].local_idefs,var)) >= 0){
					objs = var_points_to (proc,var);
					while (objs){
						aix = objs->id;
						if (addrs[aix].pid == 0){
							if (!procs[proc].global_idefs)
								procs[proc].global_idefs = create_bit_set 
									(n_globals+1);
							if (!is_bit_on(procs[proc].global_idefs,
								addrs[aix].id)){
							if (!procs[proc].global_defs)
								procs[proc].global_defs = create_bit_set 
									(n_globals+1);
								bit_on(procs[proc].global_defs,
									addrs[aix].id);
								bit_on(procs[proc].global_idefs,
			...
			if (procs[proc].lib_arefs){
				addr = -1;
				while ((addr = get_next_member (procs[proc].lib_arefs,
					addr)) >= 0){
					if (addrs[addr].pid == 0){
			...
			if(v_opt)printf ("closing %s %d changes\n",procs[proc].proc_name,
				more);
		}
	}
	more = 1;
				...
				while (call){
					if (procs[proc].global_defs){
						if (!procs[call->pid].global_defs)
								procs[call->pid].global_defs =
									create_bit_set (n_globals+1);
						result = cunion_bit_set (
							procs[call->pid].global_defs,
							procs[proc].global_defs);
						more += result;
...
scan_structs()
{
	...
	for (proc = 1; proc <= n_procs; proc++){
		locals = procs[proc].locals;
		for (i = 1; i <= procs[proc].n_locals;
				i += 1 + locals[i].offset){
			if (locals[i].offset){
				mark_sub_structs (locals,procs[proc].n_locals,i,
					locals[i].offset);
			}
		}
	}
	...
	char	*name;
{
	...
	while (fgets (buff,2000,l_file)){
		...
		case LIF_PROC_START:
			...
			for (i = procs[cur_proc].entry; i<= procs[cur_proc].exit;
				i++){
		...
		case LIF_GLOBAL_ID:
			...
			if (code == LIF_GLOBAL_ID) cur_id = globals;
			else cur_id = procs[cur_proc].locals;
			cur_id[id].name = call_malloc (strlen(id_name)+1,
				"space for id name");
			strcpy (cur_id[id].name,id_name);
			if (code == LIF_FORMAL_ID) cur_id[id].is_formal = 1;
			else cur_id[id].is_formal = 0;
			if (strchr(flags,'P')) cur_id[id].is_pointer = 1;
			else cur_id[id].is_pointer = 0;
			if (strchr(flags,'A')) cur_id[id].is_array = 1;
			else cur_id[id].is_array = 0;
			break;
		case LIF_STRUCT:
			sscanf (buff,"%*d(%d,%d,%d",&pid,&id,&offset);
			if (pid)
				procs[pid].locals[id].offset = offset;
			else globals[id].offset = offset;
			break;
		case LIF_FILE:
		...
		case LIF_AREF:
			...
			if (top_call){
				if (!call->actuals){
					actual = (actual_ptr) call_malloc (
						sizeof(actual_rec),"actual");
					actual->next = NULL;
					actual->actuals = NULL;
					call->actuals = actual;
					actual->actual_number = 1;
				}
				else actual = call->actuals;
				actual -> actuals = add_to_var_set (actual->actuals,
					id,level,code);
				if ((code == LIF_AREF) && is_lib_proc(call->pid)){
					if (procs[cur_proc].lib_arefs == NULL)
						procs[cur_proc].lib_arefs = create_bit_set (
							n_addrs+1);
				...
				if (code == LIF_GDEF){
					if (procs[cur_proc].global_defs == NULL)
						procs[cur_proc].global_defs = create_bit_set (
							n_globals+1);
					if(level){
						if (procs[cur_proc].global_idefs == NULL)
							procs[cur_proc].global_idefs =
								create_bit_set (n_globals+1);
						bit_on (procs[cur_proc].global_idefs,id);
					}
					else bit_on (procs[cur_proc].global_defs,id);
				}
					...
					if (level){
						if (procs[cur_proc].local_idefs == NULL)
							procs[cur_proc].local_idefs =
								create_bit_set (
									procs[cur_proc].n_locals+1);
						bit_on (procs[cur_proc].local_idefs,id);
					}
				}
				else if (code == LIF_CDEF){
					if (procs[cur_proc].cdefs == NULL)
						procs[cur_proc].cdefs = create_bit_set (
							n_chains+1);
					bit_on (procs[cur_proc].cdefs,id);
				}
				if ( (code == LIF_DEF) ||
					 (code == LIF_GDEF) ||
					 (code == LIF_CDEF) ) add_to =
						&files[cur_file].stmts[stmt].defs;
				else add_to = &files[cur_file].stmts[stmt].refs;
		...
		case LIF_ADDRESS:
			...
			break;
		case LIF_FIELD:
			sscanf (buff,"%*d(%d,%d,%d,%[^)]",&chain,&field_seq,&offset,
				id_name);
			field = (field_ptr) call_malloc (sizeof(field_rec),
				"allocating field");
			field->next = chains[chain].fields;
			if (chains[chain].fields && 
				(chains[chain].fields->fid < field_seq)){
				at_field = chains[chain].fields;
				while (at_field -> next &&
						(at_field->next->fid < field_seq))
			...
			field -> offset = offset;
			field -> fid = field_seq;
			field -> name = call_malloc (strlen(id_name)+1,
				"field name");
		...
		case LIF_SUCC:
			...
			files[cur_file].stmts[stmt].succ = add_to_set (
				files[cur_file].stmts[stmt].succ,id);
			break;
		case LIF_REQUIRES:
			n = sscanf (buff,"%*d(%d,%d,%d",&stmt,&from,&to);
			if (n == 2) to = from;
			for (i = from; i <= to; i++){
				files[cur_file].stmts[i].requires = add_to_set (
				files[cur_file].stmts[i].requires,stmt);
			}
			break;
		case LIF_CALL_START:
			...
			call_stack[top_call] = call = (call_ptr) call_malloc (
				sizeof(call_rec),"call site");
	...
	while(s){
		...
		*/
		...
		case LIF_GDEF:
			...
			break;
		case LIF_AREF:
			printf ("&%s ",addrs[s->id].pid?
				p->locals[addrs[s->id].id].name:
				globals[addrs[s->id].id].name);
			break;
		case LIF_CREF:
		case LIF_CDEF:
			if (chains[s->id].pid &&
				(&procs[chains[s->id].pid] != p)){
			  fprintf(stderr,"local chain %d on %s of %s used in %s\n",
					s->id,var_name (chains[s->id].pid,chains[s->id].id),
					procs[chains[s->id].pid].proc_name,
					p->proc_name);
			}
			printf ("(%d)%s",s->id,chains[s->id].pid?
				procs[chains[s->id].pid].locals[chains[s->id].id].name:
				/*
	...
	int nc,proc;
{
	...
	id_set_ptr	others;
		...
		if (procs[i].global_defs){
			at = -1;
			while ((at = get_next_member(procs[i].global_defs,at)) >=
				0){
				if (nc == 0) printf ("\t\t");
		...
		if (procs[i].other_defs){
			...
			while (others){
				if (nc == 0) printf ("\t\t");
				nc += printf ("%s ",
					procs[others->pid].locals[others->id].name);
				if (nc > 60){
					nc = 0;
					printf ("\n");
				}
		...
		if (procs[i].global_idefs){
			at = -1;
			while ((at = get_next_member(procs[i].global_idefs,at)) >=
				0){
				if (nc == 0) printf ("\t\t");
				nc += printf ("*%s ",globals[at].name);
				if (nc > 60){
		...
		if (procs[i].local_idefs){
			at = -1;
			while ((at = get_next_member(procs[i].local_idefs,at)) >=
				0){
				if (nc == 0) printf ("\t\t");
	...
	int	l_opt;
{
	...
	for (i = 0; i < n_files; i++){
		...
		files[i].name,files[i].n_procs,files[i].n_stmts,
		files[i].n_lines,files[i].n_chars);
	}
	printf ("Procs\n");
	for (i = 1; i <= n_procs; i++)if (procs[i].entry != -1){
		printf ("\t%-20s %3d  %3d %c [%d..%d] (%s)\n",
			procs[i].proc_name,i,procs[i].n_locals,
			procs[i].s_or_x,procs[i].entry,procs[i].exit,
	...
	for (i = 1; i <= n_procs; i++)if (procs[i].entry != -1){
		...
		if (!procs[i].global_defs &&
			!procs[i].other_defs &&
			!procs[i].global_idefs &&
			!procs[i].local_idefs )printf ("none");
		if (nc) printf ("\n");
	}
	printf ("Calls\n");
	for (i = 1; i <= n_procs; i++)
		...
		while (call){
			printf ("\t%4d %s: (",
				call->stmt,procs[call->pid].proc_name);
			actual = call->actuals;
			while (actual){
	...
	for (i = 1; i <= n_globals; i++){
		printf ("\t%3d %c %c %c %s",
			...
			globals[i].name);
		if (globals[i].offset) printf (" struct with %d members\n",
			globals[i].offset);
		else printf ("\n");
	}
	printf ("Locals\n");
	...
	for (i = 1; i <= n_addrs; i++){
		...
		addrs[i].pid?procs[addrs[i].pid].locals[addrs[i].id].name:
		globals[addrs[i].id].name,addrs[i].pid,addrs[i].id);
	}
	printf ("Chains\n");
	for (i = 1; i <= n_chains; i++){
		printf ("%4d ==> (%d,%d) (%s) %s",i,
			chains[i].pid,chains[i].id,
			chains[i].pid?
				procs[chains[i].pid].proc_name:
				"global",
			chains[i].pid?
				procs[chains[i].pid].locals[chains[i].id].name:
	...
	for (i = 1; i <= n_procs; i++)if (procs[i].entry != -1){
		...
		for (j = procs[i].entry; j <= procs[i].exit; j++){
			...
			for (;nc < 12; nc++) printf (" ");
			line = files[procs[i].file_id].src_index[
				files[fid].stmts[j].froml];
			while (*line != '\n') printf ("%c",*line++);
			if (files[fid].stmts[j].joins != 1)
				printf ("\n\t     %d pred",files[fid].stmts[j].joins);
			if (files[fid].stmts[j].calls) printf ("\n\t     Calls:");
			...
			printf ("\n\t     Refs: ");
			if (files[fid].stmts[j].is_return) printf ("returns ");
			if (procs[i].returns_xpr) printf ("expression ");
	...
	int		file,line;
{
	...
	for (i = 1; i <= files[file].n_stmts; i++){
		if (files[file].stmts[i].froml == line) return i;
	}
	best_line = -1;
	best_stmt = 1;
	for (i = 1; i <= files[file].n_stmts; i++){
