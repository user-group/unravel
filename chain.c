	...
	type_ste_ptr	t;
{
	if (!t) return NULL;
	switch (t->detail){
	case STE_TYPEDEF : return type_member(t->type_entry);
	case STE_TAGGED_STRUCT :
		return t->tag_entry?t->tag_entry->vars:NULL;
	case STE_ANON_STRUCT : return t->anon_entry;
	}
	return NULL;
}
	...
	var_ste_ptr	var;
{
	...
	*/
	switch (var->detail){
	case STE_TYPEDEF : return type_member(var->type_entry);
	case STE_TAGGED_STRUCT :
		return var->tag_entry?var->tag_entry->vars:NULL;
	case STE_ANON_STRUCT : return var->anon_entry;
	}
	return NULL;
}
	...
	switch (t->op_code){
		case DOT_OP:
			...
			*/
				...
				if(!c->no_output){fprintf (outfile,
						"%d(%d,%d,%d,%s)",LIF_FIELD,
						c->chain_id,c->current_seq,var->id,
						var->token->text);
			...
			*/
		case ID_OP:
			if(t->op_code == ID_OP)var =
				look_up_id ((var_ste_ptr)NULL,t->token->text,&scope);
			if (scope > 1) code = LIF_CHAIN;
			else code = LIF_GCHAIN;
			if (var == NULL) return 1;
			c->fields = find_members(var);
			/*
			printf ("fields %d\n",c->fields);
			*/
			if (c->fields == NULL) return 1;
			if (!c->no_output){
				fprintf (outfile,"%d(%d,%d)",code,c->chain_id,
					var->id);
				if (z_opt)fprintf(outfile,"    %s chain %d on %s",
		...
		case POINTER_OP:
			...
			c->fields = find_members(var);
			if (!c->no_output){
				fprintf (outfile,"%d(%d,%d,%d,%s)",LIF_FIELD,
					c->chain_id,c->current_seq,var->id,
					var->token->text);
				if (z_opt)fprintf(outfile,
					"    field %d (%s) of chain %d",
	...
	case POINTER_OP:
		...
		*/
		if ((c->chain_id == -1) || (c->chain_id == 0)){
			if (c->no_output) c->chain_id = 0;
			else c->chain_id = ++max_chain;
			c->is_global = -1;
			c->current_seq = -1;
			c->type = NULL;
			c->fields = NULL;
