	...
	chain_ptr	a,b;
{
	...
	if (!a || !b) return 0;
	if (a->pid) return 0; /* KLUDGE */
	if ((a->new_id != b->new_id) ||
		(a->pid != b->pid)) return 0;
	af = a->field;
	bf = b->field;

	while (af && bf){
		if (af -> fid != bf -> fid) return 0;
	...
	char	*dir;
{
	...
	for (i = 0; i < n; i++){
		...
		f[i].addrs = (addr_ptr) malloc ((1+f[i].n_addrs)*
			sizeof(addr_rec));
		f[i].chains = (chain_ptr) malloc((1+f[i].n_chains)*
			sizeof(chain_rec));
		...
		while (fgets (buff,2000,lif_file)){
			...
			case LIF_FIELD :
				...
				else f[i].chains[chain].field = field;
				break;
			case LIF_ADDRESS :
			...
			case LIF_PROC_START :
				...
				for (pix = 0; pix < f[i].n_procs; pix++)
					if (f[i].procs[pix].proc_id == pid) break;
				break;
			case LIF_PROC_END :
				pid = 0;
				break;
	...
	*/
	for (i = 0; i < n; i++){
		...
		if (!t_file){
			...
			f[i].procs = (proc_ptr)
				malloc (f[i].n_procs*sizeof(proc_rec));
			for (j = 0; j < f[i].n_procs; j++){
				nr = fscanf (t_file,"%d %d %d %d %c %s",
					&f[i].procs[j].proc_id,
					&f[i].procs[j].entry,
					&f[i].procs[j].exit,
			...
			nr = fscanf (t_file,"%d%d%d %d %*d %d",&f[i].n_globals,
				&f[i].n_chains,&f[i].n_addrs,
				&f[i].n_lines,&f[i].n_chars);
			if ((nr != 5) && warning) {
				warning = 0;
				fprintf (stderr,
			"Warning (slink): %sT corrupt (counts from wc missing)\n",
					f[i].name);
				}
			fclose(t_file);
			f[i].globals = (global_ptr) malloc((1+f[i].n_globals)*
				sizeof(global_rec));
	...
	char	*target;
{
	...
	while (fgets (buff,2000,sys)){
		if (buff[0] == 'M'){
			...
			if (strcmp (name,target) == 0) return n;
		}
	}
	return 0;
}
	...
	file_rec f[];
{
	...
	for (i = 0; i < n; i++){
		printf ("\nFile: %s  globals %d addrs %d chains %d procs %d\n",
			f[i].name,f[i].n_globals,f[i].n_addrs,f[i].n_chains,
			f[i].n_procs);
		printf("Globals:\n");
		...
		for (j = 1; j <= f[i].n_chains; j++){
			if (j != f[i].chains[j].old) printf ("Chain skipped\n");
			else {
		...
		for (j = 0; j < f[i].n_procs; j++){
			...
			if (localx){
				...
				while(localx){
					k += printf ("(%d => %d %s %s) ",
						localx->local,localx->global,
						localx->text,localx->flags);
					if (k > 50) {printf ("\n"); k = 0;}
					localx = localx -> next;
				}
				if (k)printf("\n");
	...
	file_rec f[];
{
	...
	for (pix = 0; pix < f[i].n_procs; pix++)
		if (f[i].procs[pix].proc_id == pid) return pix;
	return -1;
}

int find_proc (np,procs,name)
	int		np;
	proc_ptr *procs;
	char	*name;
{
	...
	for (i = 1; i <= np; i++){
		if (strcmp (name,procs[i]->name) == 0)
			if (procs[i]->s_or_x == 'X') return i;
	}
	return 0;
}

int	find_global (ng,g,name)
	int			ng;
	global_rec	g[];
	char		*name;
{
	...
	for (i = 0,k = 1; i < ng; i++,k++){
		if (strcmp (name,g[k].text) == 0)
			if (!g[k].is_static) return k;
	}
	return 0;
}

proc_ptr *assign_procs(n_files,f)
	int			n_files;
	file_rec	f[];
{
	int			total = 0;
	...
	last = 0;
	for (i = 0; i < n_files; i++) total += f[i].n_procs;
	procs = (proc_ptr *) malloc (sizeof(proc_ptr)*(++total));
	for (i = 0; i < total; i++) procs[i] = NULL;
	...
	for (i = 1; i < n_files; i++){
		for (j = 0; j < f[i].n_procs; j++){
			if (f[i].procs[j].s_or_x == 'S') at = 0;
			else at = find_proc (last,procs,f[i].procs[j].name);
			if (at) {
	...
	addr_ptr *a;
{
	...
	for (i = 1; i <= n; i++){
		gvar = a[i]->new_id;
		if ((a[i]->pid == 0) && (gvar == var)) return i;
	}
	return 0;
}

addr_ptr *assign_addrs(n_files,f)
	int			n_files;
	file_rec	f[];
{
	int			total = 0;
	...
	for (i = 0; i < n_files; i++) total += f[i].n_addrs;
				...
				if ((pix < f[i].n_procs) && (f[i].procs[pix].localx)){
					...
					while (lx){
						if (lx->local == f[i].addrs[j].id){
							...
							addr_found =
								find_global_addr(last,addrs,real_id);
							if (addr_found){
	...
	file_rec	f[];
{
	...
	if (!lx) return 0;
	while (lx){
		if (lx->local == id) return lx->global;
		lx = lx -> next;
	}
	return 0;
}

chain_ptr *shrink_chains(n_files,f)
	int			n_files;
	file_rec	f[];
{
	...
	for (i = 0; i < n_files; i++){
		for (j = 1; j <= f[i].n_chains; j++){
			if (!f[i].chains[j].pid){
				f[i].chains[j].new_id =
					f[i].globals[f[i].chains[j].id].new;
			}
			else {
				pid = f[i].chains[j].pid;
				id = f[i].chains[j].id;
				global = really_global (f,i,
					pid_to_index (n_files,f,i,pid),id);
				if (global){
					f[i].chains[j].pid = 0;
					f[i].chains[j].new_id = global;
				}
				else f[i].chains[j].new_id = f[i].chains[j].id;
			}
			for (k = 1; k <= at; k++){
				if (chains_are_equal(c[k],
						&f[i].chains[j])){
...
global_ptr assign_globals(n_files,f)
	int			n_files;
	file_rec	f[];
{
	int			total = 0;
	...
	for (i = 0; i < n_files; i++) total += f[i].n_globals;
	...
	*/
	...
	for (i = 0; i < n_files; i++){
		for (j = 0,k = 1; j < f[i].n_globals; j++,k++){
			...
			if ((static_s) &&
				(static_s - f[i].globals[k].flags <
					right - f[i].globals[k].flags)) at = 0;
			else
				at = find_global (last,globals,f[i].globals[k].text);
			if (at){f[i].globals[k].new = at; }
			else {
				last++;
			...
			if (f[i].procs[j].localx){
				...
				while (localx){
					for (k = 1; k <= f[i].n_globals; k++){
						if (0 == strcmp (f[i].globals[k].text,
							localx->text)){
						/* found */
	...
	char		*dir;
{
	...
	for (i = 1; i <= last_global; i++){
		fprintf (out_file,"%d(%d,%s%s\n",LIF_GLOBAL_ID,globals[i].new,
			globals[i].text,globals[i].flags);
	}
	for (i = 0; i < n; i++){
		...
		while (fgets(buff,2000,lif_file)){
			if (sscanf(buff,"%d",&code) == 0) code = 0;
			switch (code){
			case LIF_GLOBAL_ID :
				break;
			...
			case LIF_STRUCT :
				...
				if (node){
					...
					fprintf (out_file,"%d(%d,%d%s\n",code,
						f[i].procs[px].new_id,id,flags);
				}
				else {
					fprintf (out_file,"%d(%d,%d%s\n",code,node,
						f[i].globals[id].new,flags);
				}
				break;
			case LIF_AREF :
				sscanf (buff,"%*d(%d,%d%[^\n]",&node,&id,flags);
				fprintf (out_file,"%d(%d,%d%s\n",code,node,
					f[i].addrs[id].new_addr,
					flags);
				break;
			...
			case LIF_DEF :
				...
				if (gid){
					fprintf (out_file,"%d(%d,%d%s\n",
						code == LIF_REF?LIF_GREF:LIF_GDEF,node,
						gid,flags);
				}
				else {
					fputs(buff,out_file);
			...
			case LIF_GCHAIN :
				...
				if (chains[chain]->done) break;
				chains[chain]->done = 1;
				if (code == LIF_CHAIN)
					gid = really_global(f,i,pid_to_index(n,f,i,pid),id);
				else gid = 0;
				fprintf (out_file,"%d(%d,%d%s\n",
					gid?LIF_GCHAIN:code,chain,
	...
	global_rec	g[];
{
	...
	fprintf (k_file,
		...
		last_global,last_proc,last_addr,last_chain,n_h,n);
	for (i = 1; i <= last_proc; i++){
		fprintf (k_file,"%3d %4d %4d %4d %c %s\n",
			procs[i]->new_id,
			procs[i]->entry,
			procs[i]->exit,
			procs[i]->n_locals,
	...
	fprintf (k_file,"\n");
	for (i = 0; i < n; i++){
		fprintf (k_file,"%3d %3d %5d %4d %6d %s\n",
			i,f[i].n_procs,f[i].n_stmts,f[i].n_lines,f[i].n_chars,
			f[i].name);
	}
	new_h = h_list;
	while (new_h){
