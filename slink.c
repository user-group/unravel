...
int
chains_are_equal(chain_ptr a, chain_ptr b)
{
    field_ptr af, bf;
...
void
get_lif_files(int n, file_rec f[], char *dir)
{
    int i, j;
    int pid = 0, pix;
...
void
get_t_files(FILE *sys, int n, file_rec f[], char *dir)
{
    int i, j, len, nr;
    char buff[2000];
    FILE *t_file;
...
int
scan_system(FILE *sys, char *target)
{
...
int
pid_to_index(int n, file_rec f[], int i, int pid)
{
    int pix;
...
int
find_proc(int np, proc_ptr *procs, char *name)
{
    int i;
    for (i = 1; i <= np; i++)
    {
...
int
find_global(int ng, global_rec g[], char *name)
{
    int i;
    int k;
...
proc_ptr *
assign_procs(int n_files, file_rec f[])
{
    int total = 0;
    int i, j, last, at;
    proc_ptr *procs;
...
int
otn_globals(file_rec f[], int fid, int old)
{
    return f[fid].globals[old].new;
}

int
find_global_addr(int n, addr_ptr *a, int var)
{
    int i;
    int gvar;
...
addr_ptr *
assign_addrs(int n_files, file_rec f[])
{
    int total = 0;
    addr_ptr *addrs;
    int i, j, last = 0;
...
int
really_global(file_rec f[], int i, int j, int id)
{
    local_ptr lx;
...
chain_ptr *
shrink_chains(int n_files, file_rec f[])
{
...
global_ptr
assign_globals(int n_files, file_rec f[])
{
    int total = 0;
    int i, j, k = 0, last, at;
    global_rec *globals;
...
int
look_up_var(int i, file_rec f[], char *name)
{
    int k;
...
void
merge_head(int n, file_rec f[], char *dir, proc_ptr procs[], global_rec g[])
{
    char buff[2000], name[2000];
    FILE *h_file, *k_file;
