...
int slice_on_var = 0;
int dice_on_var  = 0;
bit_set *slice_set, *dice_set, *alt_slice_set, *primary_set, *secondary_set;
...
update_label(proc_state_ptr state)
{
        ...
        else
        {
                ...
                sprintf(buff,
                        ...
                        "<global var>",
                        "Var:",
                        state->slice_var_index ? globals[state->slice_var_index].name : "<none>");
                XtVaSetValues(*(state->state_label), XtNlabel, (XtArgVal)buff, NULL);
        }
        /*
...
adjust_popup_list(Widget list)
{
        ...
        if (bar)
                XtVaGetValues(bar, XtNwidth, &bar_w, NULL);
        else
                bar_w = 16;
        XtVaGetValues(list, XtNwidth, &list_w, XtNlongest, &list_max, XtNheight, &list_h, NULL);

        XtVaSetValues(shell,
                      XtNheight,
                      list_h > 450 ? 430 : list_h + 5,
                      XtNwidth,
                      list_max + 14 + bar_w + 5,
                      NULL);
        XtManageChild(list);
...
slicecall(Widget x, proc_state_ptr state, MultiSliceReturn *r)
{
        ...
        if (state->pid)
                var_name = procs[state->pid].locals[state->slice_var_index].name;
        else
                var_name = globals[state->slice_var_index].name;
        /*
           printf ("%s var %s\n",state->pid?"local":"global",var_name);
         */
        stmt = line_to_stmt(file, line);
        sprintf(label,
                "computing slice on %s at line %d (%d of %sc)",
                var_name,
                r->line,
                line,
                files[file].name);
        XtVaGetValues(state->display_label, XtNforeground, &save_fg, XtNbackground, &save_bg, NULL);
        ...
        sprintf(label,
                "%s(%s) %d(%sc)%s",
                var_name,
                state->pid ? procs[state->pid].proc_name : "global",
                line,
                files[file].name,
                abandon_slice ? " (partial)" : "");
        strcpy(r->message ? state->stext : state->ptext, label);
        sprintf(label, "Primary: %s   Secondary: %s", state->ptext, state->stext);
...
proc_pop(Widget a, Widget p, XtPointer c)
{
        ...
        XtTranslateCoords(top, (Position)30, (Position)60, &x, &y);
        /*
           XtVaSetValues (p,
                XtNx,	(XtArgVal) x,
                XtNy,	(XtArgVal) y,
...
head_select(Widget a, proc_state_ptr state, XawListReturnStruct *list)
{
        ...
        XawListChange(state->var_list, global_vars[list->list_index], 0, 0, True);
        state->pid             = 0;
...
proc_select(Widget a, proc_state_ptr state, XawListReturnStruct *list)
{
        ...
        XtUnmanageChild(state->var_list);
        XawListChange(state->var_list, local_vars[list->list_index], 0, 0, True);
        adjust_popup_list(state->var_list);
        XtManageChild(state->var_list);
        proc_pop(NULL, state->var_popup, NULL);
        /*
...
char *fall[] = {
        ...
        "*title: Unravel Version 2.3 Program Slicer",
...
main(int argc, char **argv)
{
        ...
        fall[0]  = icon_res;
        top      = XtAppInitialize(&ac, "Unravel", opt, XtNumber(opt), &argc, argv, fall, NULL, 0);
...
create_widgets(Widget top, char *file_name)
{
        ...
        static XtResource res[] = {{XtNrunningFG,
                                    ...
                                    XtRPixel,
                                    sizeof(Pixel),
                                    XtOffsetOf(proc_state_rec, active_fg),
                                    XtRString,
                                    "red"},
                                   {XtNrunningBG,
                                    XtCForeground,
                                    XtRPixel,
                                    sizeof(Pixel),
                                    XtOffsetOf(proc_state_rec, active_bg),
                                    XtRString,
                                    "yellow"}};
        ...
        /*
           file_menu = XtVaCreatePopupShell ("fileMenu",simpleMenuWidgetClass,
                ...
                NULL);
         */
        select_menu = XtVaCreatePopupShell("selectMenu", simpleMenuWidgetClass, top, NULL);
        /*
           set_button_help (file_menu,"Quit to exit unravel");
         */
        set_button_help(select_menu, "Menu for selecting criteria variables or previous slices");
        op_menu = XtVaCreatePopupShell("opMenu", simpleMenuWidgetClass, top, NULL);
        set_button_help(op_menu, "Menu for combining primary and secondary slices");
        select_popup = XtVaCreatePopupShell("selectpop", transientShellWidgetClass, top, NULL);
        select_port  = XtVaCreateManagedWidget("selectport",
                                               ...
                                               (XtArgVal)True,
                                               NULL);
        select_list  = XtVaCreateManagedWidget(
                "selectlist", listWidgetClass, select_port, XtNdefaultColumns, (XtArgVal)1, NULL);
        set_button_help(select_list, "list of already computed slices");
        ps.history_list = select_list;
        head_popup      = XtVaCreatePopupShell("headpop", transientShellWidgetClass, top, NULL);
        head_port       = XtVaCreateManagedWidget("headport",
                                                  ...
                                                  (XtArgVal)True,
                                                  NULL);
        proc_popup      = XtVaCreatePopupShell("procpop", transientShellWidgetClass, top, NULL);
        proc_port       = XtVaCreateManagedWidget("procport",
                                                  ...
                                                  (XtArgVal)True,
                                                  NULL);
        var_popup       = XtVaCreatePopupShell("varpop", transientShellWidgetClass, top, NULL);
        ...
        select_button = XtVaCreateManagedWidget("Select",
                                                ...
                                                (XtArgVal)XtChainLeft,
                                                NULL);
        set_button_help(select_button, "Menu for selecting criteria variables or previous slices");
        proc = XtVaCreateManagedWidget(
                "Procedure", smeBSBObjectClass, select_menu, XtNlabel, (XtArgVal) "Local Variable", NULL);
        XtAddCallback(proc, XtNcallback, proc_pop, proc_popup);
        var = XtVaCreateManagedWidget(
                "Variable", smeBSBObjectClass, select_menu, XtNlabel, (XtArgVal) "Global Variable", NULL);
        XtAddCallback(var, XtNcallback, proc_pop, head_popup);
        show_proc = XtVaCreateManagedWidget(
                "Markproc", smeBSBObjectClass, select_menu, XtNlabel, "Mark proc", NULL);
        show_tree = XtVaCreateManagedWidget(
                "ShowCallTree", smeBSBObjectClass, select_menu, XtNlabel, "Show call tree", NULL);
        primary       = XtVaCreateManagedWidget("Primary", smeBSBObjectClass, select_menu, NULL);
        secondary     = XtVaCreateManagedWidget("Secondary", smeBSBObjectClass, select_menu, NULL);
        ...
        dice   = XtVaCreateManagedWidget("Dice", smeBSBObjectClass, op_menu, NULL);
        dicesp = XtVaCreateManagedWidget(
                "Dicesp", smeBSBObjectClass, op_menu, XtNlabel, (XtArgVal) "Dice S-P", NULL);
        bb = XtVaCreateManagedWidget(
                "Backbone", smeBSBObjectClass, op_menu, XtNlabel, (XtArgVal) "Intersection", NULL);
        merge       = XtVaCreateManagedWidget("Union", smeBSBObjectClass, op_menu, NULL);
        clear       = XtCreateManagedWidget("Clear", smeBSBObjectClass, op_menu, NULL, 0);
        clear_slice = XtVaCreateManagedWidget(
                "ClearSlice", smeBSBObjectClass, op_menu, XtNlabel, (XtArgVal) "Clear Slice History", NULL);

        XtVaGetValues(op_button, XtNwidth, &w, XtNborderWidth, &bw, XtNx, &x, NULL);
        interrupt = XtVaCreateManagedWidget("interrupt",
                                            ...
                                            (XtArgVal)XtChainTop,
                                            NULL);
        set_button_help(interrupt, "Abandon computing slice, show partial result");
	...
	# of statements in slice");
        ...
        sprintf(buff, "%s %s", "File: <none> Proc: <none>", "Var: <none>");
        XtVaGetValues(frame, XtNwidth, &w, NULL);
        ...
        proc_state = XtVaCreateManagedWidget("pstate",
                                             ...
                                             (XtArgVal)XtJustifyLeft,
                                             NULL);
        set_button_help(proc_state, "Show the criteria of the primary and secondary slices");
        ...
        content    = XtVaCreateManagedWidget("info",
                                             ...
                                             frame,
                                             XtNfromVert,
                                             (XtArgVal)XtParent(slicewin),
                                             XtNright,
                                             (XtArgVal)XtChainRight,
                                             XtNleft,
                                             (XtArgVal)XtChainLeft,
                                             XtNtop,
                                             (XtArgVal)XtChainBottom,
                                             XtNbottom,
                                             (XtArgVal)XtChainBottom,
                                             XtNjustify,
                                             (XtArgVal)XtJustifyCenter,
                                             XtNlabel,
                                             (XtArgVal)buff,
                                             NULL);
        help_label = content;
        XtAddEventHandler(
                content, EnterWindowMask, False, button_help, "Describe object under mouse pointer");
        ...
        set_button_help(
                XtNameToWidget(sliceframe, "bar"), "Scroll bar to navigate through source text");
        ...
        XtVaGetValues(frame, XtNwidth, &w, XtNdefaultDistance, &dist, NULL);
        XtVaGetValues(help, XtNwidth, &www, XtNborderWidth, &bw, XtNx, &hx, NULL);
        XtVaGetValues(progress, XtNwidth, &ww, XtNx, &cx, NULL);
        ww = w - (cx + ww + 4 * bw + dist + www);
        XtVaGetValues(selection_state, XtNborderWidth, &bw, NULL);
        XtUnmanageChild(help);
        XtUnmanageChild(selection_state);
        XtUnmanageChild(proc_state);
        XtVaSetValues(selection_state, XtNwidth, (XtArgVal)(w - 2 * (dist + bw)), NULL);
        XtVaSetValues(proc_state, XtNwidth, (XtArgVal)(w - 2 * (dist + bw)), NULL);
        XtVaSetValues(help, XtNhorizDistance, ww, NULL);
        XtManageChild(selection_state);
