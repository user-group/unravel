    ...
    char **argv;
{
	...
	fall[0] = res;
	toplevel = XtAppInitialize (&ac,"Unravel",NULL,0,&argc,argv,
		fall,NULL,0);
    panes = XtCreateManagedWidget("frame", panedWidgetClass,
			   toplevel, NULL, 0);

    bbox = XtCreateManagedWidget("buttons", boxWidgetClass,
			   panes, NULL, 0);
    quit = XtCreateManagedWidget("quit", commandWidgetClass,
			   bbox, NULL, 0);
	XtAddCallback (quit,XtNcallback,quitB,NULL);
	sprintf (buff,"%s",argv[1]);
	editwin = XtCreateManagedWidget("editor", asciiTextWidgetClass,
        panes, wargs, XtNumber(wargs));
