	...
	char	*p[];
{
	...
	while (NULL != fgets (buff,2000,ff_file)){
		...
		switch (code){
		  ...
		  case LIF_PROC_END :
				...
				if (node > last_node) last_node = node;
				n_procs++;
				max_local = 0;
				max_formal = 0;
			break;
		  case LIF_FORMAL_ID :
				sscanf (buff,"%*d(%d",&id);
				if (id > max_formal) max_formal = id;
			break;
		  case LIF_LOCAL_ID :
				sscanf (buff,"%*d(%d",&id);
		  ...
		  case LIF_GLOBAL_ID :
				sscanf (buff,"%*d(%d",&id);
				if (id > last_global_id) last_global_id = id;
			break;
		  case LIF_AREF :
		  ...
		  case LIF_ADDRESS :
				sscanf (buff,"%*d(%d",&id);
				sscanf (buff,"%*d(%d",&id);
				if (id > last_addr) last_addr = id;
			break;
		  case LIF_REF :
				level = sscanf (buff,"%*d(%*d,%*d,%d",&id);
				if (level)procs[n_procs].n_irefs++;
				else procs[n_procs].n_refs++;
			break;
		  case LIF_DEF :
				level = sscanf (buff,"%*d(%*d,%*d,%d",&id);
				if (level)procs[n_procs].n_idefs++;
				else procs[n_procs].n_defs++;
			break;
		  case LIF_GREF :
				level = sscanf (buff,"%*d(%*d,%*d,%d",&id);
				if (level) procs[n_procs].n_global_irefs++;
				else procs[n_procs].n_global_refs++;
			break;
		  case LIF_GDEF :
				level = sscanf (buff,"%*d(%*d,%*d,%d",&id);
				if(level) procs[n_procs].n_global_idefs++;
				else procs[n_procs].n_global_defs++;
			break;
		  case LIF_CREF :
	...
	printf ("last_global %d last_node %d last_addr %d n_procs %d\n",
		last_global_id,last_node,last_addr,n_procs);
	printf ("%10s %10s %10s %10s %10s %10s\n",
		"proc_id","n_locals","global_refs","local_refs",
		"chain_refs",
		"has_return");
	...
	printf (" %3s %4s %4s %4s %4s %4s %4s %4s %4s %4s\n",
		"PID","iref","idef","gdef","def","gir","gid",
		"nc","cd","ar");
	for (id = 0; id < n_procs; id++){
		printf ("%3d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d =%s= %s\n",
			...
			(procs[id].n_idefs+procs[id].n_global_defs+
				procs[id].n_global_idefs+procs[id].n_cdefs)?
				"??":"OK",
			procs[id].name
			);
	}
