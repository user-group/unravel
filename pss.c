...
void print_pvs(void) 
{
    ...
    for (i = 1; i <= n_procs; i++) {
        if (procs[i].returns_xpr) {
            ...
            while (to) {
                ...
                printf(" %s", a_pid ? procs[a_pid].locals[a_id].name : globals[a_id].name);
                to = to->next;
            }
            printf("\n");
        }
    ...
    for (i = 1; i <= n_ptrs; i++) {
        ...
        printf("%s (%s) -> ",
            ids[idx]->name,
            ptrs[i].proc_id ? procs[ptrs[i].proc_id].proc_name : "global");
        to = ptrs[i].addrs;
        if (!to)
            printf("<NULL>");
        while (to) {
                ...
                if (offset_check(addrs[a_id].pid, addrs[a_id].id, f->offset)) {
                    ...
                    if (new_id <= (addrs[a_id].pid ? procs[addrs[a_id].pid].n_locals : n_globals)) {
                        ...
                        printf (" (new %d) %s",
                                ...
                                globals[new_id].name);
                        */
                        new_ptr = addrs[a_id].pid ? procs[addrs[a_id].pid].locals[new_id].ptr_id
...
void get_cdefs(int chain) 
{
    ...
    if (pid)
        ptr = procs[pid].locals[id].ptr_id;
    else
        ptr = globals[id].ptr_id;
    /*
...
int prop_call(int pid, int fid, int sid)
{
    ...
    /* printf ("%5d %5d %5d\n",pid,fid,sid); */
    d = files[fid].stmts[sid].defs;
    if (!d && !files[fid].stmts[sid].is_return)
        return 0;
