		...
		case DOT_OP:
			...
			if (c.chain_id != -1){
				...
				if (z_opt)fprintf(outfile,
					...
					c.current_seq,var->token->text,c.chain_id);
				fprintf (outfile,"\n");
				*chain_id = c.chain_id;
	...
	switch (t->op_code){
		...
		case COMMA_OP:
			...
			*/
			...
			if (lvalue == ID_LVALUE) ref_def_var (REF,new_node,tk,is_array);
			else if (lvalue == PTR_LVALUE) cref_def(REF,new_node,chain_id);
			if(t->left){
				new_from = 0; new_to = 0; new_upto = 0;
				scan_tree (t->left,new_node,
					&new_from,&new_upto,&new_to);
				/*
				printf ("new %d  from %d upto %d to %d  ",
					new_node,new_from,new_upto,new_to);
				*/
				if (!new_from) new_from = new_node;
				if (!new_upto) new_upto = new_node;
				else connect_nodes (new_upto,new_node);
				if (!new_to) new_to = new_node;
				if (*from_node) connect_nodes (new_to,*from_node);
				*from_node = new_from;
				if(!(*upto_node)) *upto_node = new_to;
			}
			/*
			printf ("node %d  from %d upto %d to %d\n",
					node_id,*from_node,*upto_node,*to_node);
			...
			else if (lvalue == PTR_LVALUE) cref_def(REF,node_id,chain_id);
			if(t->right) scan_tree (t->right,node_id,
				from_node,upto_node,to_node);
			return;
		case LEFTP_OP:
		case RIGHTP_OP:
			if(t->left)
		...
		case ASGN_OP:
			is_array = 0;
			lvalue = get_lvalue (t->left,&tk,&chain_id,&is_array);
			if (lvalue == ID_LVALUE) ref_def_var (DEF,node_id,tk,is_array);
		...
		case POST_OP:
			...
			*/
			if (*to_node == 0)connect_nodes (node_id,new_node);
			else connect_nodes (*to_node,new_node);
			*to_node = new_node;
			...
			*/
			if (lvalue == ID_LVALUE){
				...
				*/
				...
				fprintf (outfile,"%d(%d,%d)",LIF_CALL_START,
					node_id,proc_id);
				if(z_opt){
					fprintf (outfile,"     call to %s at %d",
						tk->text,node_id);
				}
				fprintf (outfile,"\n");
				assemble_actuals (t->right,node_id,
					from_node,upto_node,to_node);
				fprintf (outfile,"%d",LIF_CALL_END);
				if(z_opt)
					fprintf (outfile,"     end call to %s at %d",
						tk->text,node_id);
				fprintf (outfile,"\n");
	...
	int			*from,*to;
{
	...
	*from = *to = 0;
	...
	source_map (node_id,leftmost(t),rightmost(t));
		*/
	if(upto) connect_nodes(upto,node_id);
	if (*from == 0) *from = node_id;
	if (*to == 0) *to = node_id;
