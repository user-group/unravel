	...
	int			id,pid;
{
	...
	if (head){
		at = head;
		while (at){
			if ((at->id == id) && (at->pid == pid)) return 1;
			at = at -> next;
		}
	}
	return 0;
	...
	int			id,pid;
{
	...
	if (*head){
		at = *head;
		while (at){
			if ((at->id == id) && (at->pid == pid)) return 0;
			last = at;
			at = at -> next;
		}
	}
	...
	int		size;
{
	...
	*/
	...
	if (start) for (i = 0;i < w; i++) start[i] = 0;
...
int	is_bit_on (b,at)
	...
	int			at;
{
	if (at < 0 || !b || b->max < at) return 0;
	if (b->bits[at/WORD_SIZE] & 1<<(at%WORD_SIZE)) return 1;
	return 0;
}
	...
	bit_set		sub,set;
{
	...
	for (i = 0; i < (sub->max + WORD_SIZE - 1)/WORD_SIZE; i++) {
		if (sub->bits[i] & (~set->bits[i])) return 0;
		if (sub->bits[i] != set->bits[i]) proper = 1;
	}
	return proper;
}
	...
	int			at;
{
	...
	at--;
	if (at < 0 || !set || at > set->max) return -1;
	end = (at/WORD_SIZE)*WORD_SIZE;
	if ((set->bits[at/WORD_SIZE])<<(WORD_SIZE - (at%WORD_SIZE)))
				/* more in this word */
		for (i=at;i>=end;i--)if (is_bit_on(set,i)) return i;
	start = end/WORD_SIZE - 1;
	for (j = start; j >= 0; j--) if (set->bits[j])
		for (i = (j + 1)*WORD_SIZE - 1; i >= j*WORD_SIZE; i--)
			if (is_bit_on (set,i)) return i;
	return -1;
}
	...
	int			last;
{
	...
	last++;
	if (last < 0 || !set || last > set->max) return -1;
	end = (last/WORD_SIZE + 1)*WORD_SIZE;
	if ((set->bits[last/WORD_SIZE])>>(last%WORD_SIZE))  
				/* more in this word */
		for (i=last;i<end;i++)if (is_bit_on(set,i)) return i;
	start = end/WORD_SIZE;
	end = set->max/WORD_SIZE;
	for (j = start; j<=end; j++) if (set->bits[j])
