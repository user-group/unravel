...
int on_list(level, callers, called) int level, callers[], called;
{
    int i;
...
int is_recursive_call(level, callers, called) int level, callers[], called;
{ return on_list(level, callers, called); }

void tree_out(level, callers, calls) int level, callers[];
call_ptr calls;
{
    char tabs[DEPTH];
    int i, called;
