	...
	label_ptr	label;
{
	...
	update (ana,n_analyzed,n_analyzed == 1?"%3d file analyzed & up to date":
			"%3d files analyzed & up to date");
	update (src,n_files,n_files == 1?"%3d source file":
			"%3d source files");
	update (not,n_not,n_not == 1?"%3d source file not analyzed":
			"%3d source files not analyzed");
	update (out,n_out,n_out == 1?"%3d file analyzed & out of date":
			"%3d files analyzed & out of date");
	update (link,n_link,n_link == 1?"%3d main program file linked":
			"%3d main program files linked");
	update (prog,n_prog,n_prog == 1?"%3d main program file analyzed":
			"%3d main program files analyzed");
	update (dup,n_dup,n_dup == 1?"%3d duplicate procedure found":
	...
	XtPointer	w_data;
{
	...
	sprintf (buff,"%s/helpu %s -geom +10+10 -title \"%s\" %s &",
		HOME,"-xrm \"*Command.label: History Review Done\"",
		title[hist],file_name[hist]);
	system(buff);
}
...
{
	char	buff[2000];
	sprintf (buff,"%s/select %s >HISTORY-S 2>&1 &",HOME,
		link_only?"-noslice":"");
	system (buff);
}
	...
	label_ptr	labels;
{
	...
	if (status){
		...
		*/
		...
		update_src_info(labels);
		sprintf (buff,
			"echo -n \"Unravel change directory: %s \" >> HISTORY",
			labels->dir_buff);
		system(buff);
		system("date >> HISTORY");
	...
	XtPointer	w_data;
{
	...
	sprintf (buff,"%s/helpu -geom +10+10 %s/unravel.help %s &",HOME,HOME,
	...
	Widget	top;
{
	...
	Widget			frame,link;
	Widget			review_menu,review_ana,review_slice,
					review_all,review_this;
	...
	analysis = XtVaCreateManagedWidget ("analysis",commandWidgetClass,
		frame,
		XtNtop,				XawChainTop,
		XtNbottom,			XawChainTop,
		XtNleft,			XawChainLeft,
	...
	review_menu = XtVaCreatePopupShell ("reviewMenu",
		simpleMenuWidgetClass,top,NULL);
	
	review = XtVaCreateManagedWidget ("review",
		menuButtonWidgetClass,frame,
		XtNlabel,			"Review History",
		XtNtop,				XawChainTop,
		XtNbottom,			XawChainTop,
		XtNleft,			XawChainLeft,
		XtNright,			XawChainLeft,
		XtNfromHoriz,		analysis,
		XtNmenuName,	(XtArgVal) "reviewMenu",
		NULL);
	review_ana = XtVaCreateManagedWidget ("reviewAnalysis",
		smeBSBObjectClass,review_menu,
		XtNlabel,		(XtArgVal) "Last Analysis",
		NULL);
	review_slice = XtVaCreateManagedWidget ("reviewSlice",
		smeBSBObjectClass,review_menu,
		XtNlabel,		(XtArgVal) "Last Slice",
		NULL);
	review_this = XtVaCreateManagedWidget ("review",
		smeBSBObjectClass,review_menu,
		XtNlabel,		(XtArgVal) "This Session",
		NULL);
	review_all = XtVaCreateManagedWidget ("reviewAll",
		smeBSBObjectClass,review_menu,
		XtNlabel,		(XtArgVal) "All History",
		NULL);
	...
	slice = XtVaCreateManagedWidget ("slice",commandWidgetClass,
		frame,
		XtNtop,				XawChainTop,
		XtNbottom,			XawChainTop,
		XtNleft,			XawChainLeft,
	...
	*/
	...
	src = XtVaCreateManagedWidget ("src",labelWidgetClass,
		frame,
		XtNtop,				XawChainTop,
		XtNbottom,			XawChainTop,
		XtNleft,			XawChainLeft,
	...
	ana = XtVaCreateManagedWidget ("ana",labelWidgetClass,
		frame,
		XtNtop,				XawChainTop,
		XtNbottom,			XawChainTop,
		XtNleft,			XawChainLeft,
	...
	not = XtVaCreateManagedWidget ("not",labelWidgetClass,
		frame,
		XtNtop,				XawChainTop,
		XtNbottom,			XawChainTop,
		XtNleft,			XawChainLeft,
	...
	out = XtVaCreateManagedWidget ("out",labelWidgetClass,
		frame,
		XtNtop,				XawChainTop,
		XtNbottom,			XawChainTop,
		XtNleft,			XawChainLeft,
	...
	prog = XtVaCreateManagedWidget ("prog",labelWidgetClass,
		frame,
		XtNtop,				XawChainTop,
		XtNbottom,			XawChainTop,
		XtNleft,			XawChainLeft,
	...
	link = XtVaCreateManagedWidget ("link",labelWidgetClass,
		frame,
		XtNtop,				XawChainTop,
		XtNbottom,			XawChainTop,
		XtNleft,			XawChainLeft,
	...
	dup = XtVaCreateManagedWidget ("dup",labelWidgetClass,
		frame,
		XtNtop,				XawChainTop,
		XtNbottom,			XawChainTop,
		XtNleft,			XawChainLeft,
	...
	set_button_help (help,
		"Click to pop up a window describing unravel");
	set_button_help (ana,
		"Display how many source files have been analyzed");
	set_button_help (not,
		"Display how many source files have not been analyzed");
	set_button_help (out,
		"Display how many source files have been modified since being analyzed");
	...
	set_button_help (src,
		"Display how many source files are in the directory");
	set_button_help (dir,
...
*/
	static XtResource res[] = {
		{XtNlinkOnly,XtCValue, XtRInt, sizeof(int),
			0,XtRString,"0"}};
