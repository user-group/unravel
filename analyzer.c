...
do_final_analysis(nf_ptr nf, XtIntervalId *tt)
{
    ...
    sprintf(buff,
        "%s ; %s ;%s/map >> HISTORY-A 2>&1",
        "echo \" \" >>HISTORY-A",
        "echo running map >> HISTORY-A",
        HOME);
    system(buff);
    if (nf->errs)
        sprintf(
            buff, "Analysis complete, %d file%s not ANSI C", nf->errs, nf->errs == 1 ? "" : "s");
    else
        sprintf(buff, "Analysis complete");
    XtVaSetValues(
        nf->s, XtNlabel, buff, XtNforeground, nf->save_fg, XtNbackground, nf->save_bg, NULL);
    nf->running = 0;
    notify_unravel(nf->s);
    if (nf->r)
...
do_analysis(nf_ptr nf, XtIntervalId *tt)
{
    ...
    else
    {
        ...
        system(buff);
        sprintf(buff, "%s/tsummary %s.c >>HISTORY-A", HOME, file);
        system(buff);
        sprintf(buff, "PATH=%s:$PATH ; export PATH ; vprep %s.c", HOME, file);
        system(buff);
        if (finish - start)
            rate = c_stat.st_size / (elapsed);
        else
            rate = c_stat.st_size;
        sprintf(mess,
    ...
    sprintf(buff, "echo \"%s\" >> HISTORY-A ; %s", mess, "cat *.ERR >>HISTORY-A");
    system(buff);
    i++;
    nf->at = i;
    if (i < n)
    {
        ...
        XtVaSetValues(nf->s, XtNlabel, buff, NULL);
        return;
    }
    XtVaSetValues(nf->s, XtNlabel, "Scanning for main programs . . .", NULL);
...
run_a(Widget w, nf_ptr nf, XtPointer c)
{
    ...
    if (n > 0)
    {
        ...
        XtVaSetValues(
            nf->s, XtNlabel, buff, XtNforeground, nf->run_fg, XtNbackground, nf->run_bg, NULL);
        nf->running = 1;
        nf->r       = w;
...
push_help(Widget w, XtPointer u_data, XtPointer w_data)
{
    ...
    sprintf(buff,
        "%s/helpu -geom +20+20 %s %s/analyzer.help &",
        HOME,
        "-title \"Analyzer Help\" ",
        HOME);
    system(buff);
...
adjust_lists(Widget frame, Widget left, Widget right)
{
    ...
    XtVaGetValues(frame, XtNdefaultDistance, &default_dist, XtNwidth, &frame_w, NULL);

    XtVaGetValues(left, XtNwidth, &left_w, XtNborderWidth, &left_bw, NULL);

    XtVaGetValues(right, XtNwidth, &right_w, XtNborderWidth, &right_bw, NULL);

    w = frame_w - 2 * (left_bw + right_bw + default_dist) - default_dist;
    w = w / 2;
    XtVaSetValues(right, XtNwidth, (XtArgVal)(w), NULL);
    XtVaSetValues(left, XtNwidth, (XtArgVal)(w), NULL);
...
rj_button(Widget goal, Widget left, Widget w)
{
    ...
    XtVaGetValues(goal, XtNdefaultDistance, &dd, XtNwidth, &goal_w, NULL);

    XtVaGetValues(left, XtNborderWidth, &left_bw, XtNwidth, &left_w, XtNx, &left_x, NULL);

    XtVaGetValues(w, XtNborderWidth, &bw, XtNwidth, &button_w, XtNx, &x, NULL);
...
align(Widget label1, Widget text1, Widget label2, Widget text2)
{
    ...
    XtVaGetValues(frame, XtNdefaultDistance, &dd, NULL);
    XtVaGetValues(label1, XtNwidth, &w1, NULL);
...
make_list_box(Widget parent, char *prefix, char *title)
{
    ...
    frame = XtVaCreateManagedWidget(
        strcat(name, "Frame"), formWidgetClass, parent, XtNborderWidth, 2, NULL);
...
make_windows(Widget top, XtAppContext ac)
{
    ...
    static char *file_names[3000], *pick_names[3000], *skip_names[3000] = {NULL};
    ...
    opt_menu = XtVaCreatePopupShell("optMenu", simpleMenuWidgetClass, top, NULL);

    opt_button = XtVaCreateManagedWidget("selections",
        menuButtonWidgetClass,
        frame,
        XtNlabel,
        (XtArgVal) "File Selection",
        XtNtop,
        (XtArgVal)XawChainTop,
        XtNbottom,
        (XtArgVal)XawChainTop,
        XtNborderWidth,
        (XtArgVal)2,
    ...
    */
    select_all = XtVaCreateManagedWidget(
        "selectAll", smeBSBObjectClass, opt_menu, XtNlabel, (XtArgVal) "Select All Files", NULL);
    select_not  = XtVaCreateManagedWidget("selectNot",
        smeBSBObjectClass,
        opt_menu,
        XtNlabel,
        (XtArgVal) "Select Files Not Analyzed",
        NULL);
    select_none = XtVaCreateManagedWidget(
        "selectNone", smeBSBObjectClass, opt_menu, XtNlabel, (XtArgVal) "Select No Files", NULL);
    select_ana = XtVaCreateManagedWidget("selectAnalyzed",
        smeBSBObjectClass,
        opt_menu,
        XtNlabel,
        (XtArgVal) "Select Analyzed Files",
        NULL);
    ...
    */
    op_button = XtVaCreateManagedWidget("operations",
        ...
        frame,
        XtNlabel,
        (XtArgVal) "Analyze Selected Files",
        XtNtop,
        (XtArgVal)XawChainTop,
        XtNbottom,
        (XtArgVal)XawChainTop,
    ...
    cpp_label = XtVaCreateManagedWidget("cppLabel",
        labelWidgetClass,
        frame,
        XtNlabel,
        "CPP flags: ",
    ...
    parse_label = XtVaCreateManagedWidget("parserLabel",
        labelWidgetClass,
        frame,
        XtNlabel,
        "Parse flags: ",
    ...
    XtAddCallback(XtNameToWidget(not_selected, "*notSelectList"), XtNcallback, ilist, &names);
    ...
    XtVaGetValues(cpp_text, XtNwidth, &W, XtNheight, &H, NULL);
    H += 20;
    XtUnmanageChild(cpp_text);
    XtVaSetValues(cpp_text, XtNheight, H, NULL);
    XtManageChild(cpp_text);

    XtVaGetValues(parse_text, XtNwidth, &W, XtNheight, &H, NULL);
    H += 20;
    XtUnmanageChild(parse_text);
    XtVaSetValues(parse_text, XtNheight, H, NULL);
    ...
    help_label = info;
    set_button_help(opt_button, "Use this menu to select all files, no files or analyzed files");
    set_button_help(clear, "Click to remove analysis of selected files");
    set_button_help(op_button, "Click to run analysis on selected files");
    set_button_help(info, "Description of object under mouse pointer is displayed here");
    set_button_help(quit, "Click to exit analyzer");
    set_button_help(help, "Click to pop up a window describing analyzer");
    ...
    set_button_help(cpp_label, "Flags for running the C preprocessor (CPP)");
    set_button_help(status, "Display status as analysis runs");
    set_button_help(
        XtNameToWidget(selected, "*vertical"), "use middle mouse button to slide scroll bar");
    set_button_help(
        XtNameToWidget(not_selected, "*vertical"), "use middle mouse button to slide scroll bar");
    ...
    set_button_help(XtNameToWidget(selected, "selectLabel"), "List of files selected for analysis");
    set_button_help(
        XtNameToWidget(not_selected, "notSelectLabel"), "List of files not to be analyzed");
}
