...
void print_st() {
    ...
    for (i = 0; i < n_st; i++) {
        ...
        while (vars) {
            ...
            printf("%03d ", vars->id);
            printf("F(%07o) <%d>  ", vars->type_decl->desc.decl->decl_flags, vars->detail);
            /*
            ...
            */
            ...
            if (vars->tag_entry)
                printf("{%s} ", vars->tag_entry->tag->desc.decl->tag->text);
            if (vars->type_entry)
...
token_ptr fake_var_decl(t) token_ptr t;
{
    ...
    if (c_opt)
        printf("make malloc at %d on line %d\n", stmtno, lineno);
    ...
    if ((!t) || ((np = is_ptr_to_ptr(t, &nx)) > 1)) {
        ...
        if (nx)
            ...
            if (!type_token)
                ...
                ty->decl_flags = (type_token->desc.decl->decl_flags) & (~FLAG_SC_TYPEDEF);
